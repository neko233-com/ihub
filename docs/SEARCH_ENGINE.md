...
- 三栏工作台保留 uTools 本地搜索的高频键盘路径：`Tab` / `Shift+Tab` 循环文件类别，`↑` / `↓` 移动当前项，按住 `Shift` 扩展连续多选，`Enter` 打开当前项，`→` 或鼠标右键打开文件菜单，`Ctrl+C` 把最多 64 个所选项目作为原生文件列表复制到系统剪贴板。复制命令只接收不透明结果 ID，并在宿主中重新解析、验证当前索引范围与文件身份；renderer 显示的路径从不直接成为剪贴板授权。
