...
`redirectHotKeySetting(commandLabel, autoCopy?)` 和 `redirectAiModelsSetting()` 会从当前可见活动插件跳转到 iHub 设置内对应的快捷键或 AI 提供方区段；快捷键调用保留并校验官方参数，但 iHub 当前只定位到自己的启动快捷键设置，不伪装成已提供逐插件命令快捷键编辑器。BrowserWindow、隐藏 runtime 或失效租约不能触发该导航。
