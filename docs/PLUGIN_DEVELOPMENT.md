...
`copyImage` 和 `hideMainWindowPasteImage` 除了有界 PNG Data URL/`Uint8Array`，也接受同一插件、同一 surface 由 `showOpenDialog` 返回的图片路径。宿主在读取前复验 picker lease、文件类型与对象身份，最多读取 4 MiB，并只解码 PNG、JPEG 或 WebP；未授权路径在文件系统探测前拒绝。
