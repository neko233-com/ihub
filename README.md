...
- **离线翻译**：独立双栏工作台默认随应用提供中英双向基础包，自动识别已安装语言包覆盖的文字脚本并选择另一侧目标语言；输入、结果和词典匹配全部在 WebView 内完成，状态栏明确显示“网络请求 0”。自定义包优先覆盖内置术语，没有直连语言对时可通过 English 本地枢轴路由。其他语言可导入最多 1 MiB / 5,000 词条的纯 JSON 本地语言包，单机最多保留 8 个、总量 2 MiB，容量按 UTF-8 字节计算；包不会执行代码，也没有云端回退。词典覆盖率和未覆盖片段会如实显示，格式见[离线翻译语言包](docs/OFFLINE_TRANSLATION.md)。
...
- **Wi-Fi 密码查看器**：Windows Native Wi-Fi API 只列出已保存配置的名称、适配器和安全元数据；用户必须对单个预共享密钥配置点击请求并批准 UAC。管理员辅助模式通过随机本机命名管道返回一次密钥，父进程核对真实客户端 PID；原生 XML/JSON 缓冲主动清零，WebView 只显示 60 秒。它不调用 `netsh` / PowerShell、不读取企业 802.1X 凭据、不修改 WLAN；详细边界见[Wi-Fi 密码查看器](docs/WIFI_PASSWORD_VIEWER.md)。
...
也可导入公开 uTools <code>plugin.json</code>：<code>main</code>、<code>logo</code> 与文本型 <code>features</code> 会投影为 iHub 前端命令；<code>tools</code> 会按有界 JSON Schema 锁定，并由当前隐藏 runtime 的 <code>utools.registerTool</code> 注册，支持结构化调用、进度、取消、超时和生命周期清理。tools-only 包可只声明 <code>preload + tools</code>；声明的 preload 会纳入 SHA-256 快照，并从宿主内存路由作为无 Node 权限的普通 WebView 脚本加载。兼容层还提供 ready/enter/out 生命周期、宿主子输入框、动态功能、同步文档/附件数据库、取色与选区截图、显示器 API、经系统选择器授权的录屏源、受确认保护的本地 Shell 动作、Windows 原生文件图标、宿主有界 <code>utools.sharp</code> 图像管线、受系统文件选择授权的 <code>utools.runFFmpeg</code>，以及可配置 OpenAI-compatible Chat Completions 服务的 <code>utools.ai</code> / <code>utools.allAiModels</code>。FFmpeg 首次使用须由用户确认，固定下载 Gyan FFmpeg 8.1.2 essentials GPLv3 构建并校验归档与可执行文件 SHA-256；插件不能传入网络、管道或未经选择器授权的本机路径。AI Provider 的地址、模型和默认项由用户在偏好设置中管理，API key 只进入系统凭据保护的加密存储；流式正文、推理内容、函数工具调用、取消、轮次上限和插件生命周期清理均由 Rust 宿主控制。该沙箱不提供 <code>fs</code>、<code>child_process</code> 或其他 Node/Electron ambient authority；文件/图片 matcher 与浏览器可访问的本地搜索索引也不会因此开放。本地搜索继续完全由 Rust 原生索引实现。
