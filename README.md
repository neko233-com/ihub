...
也可导入公开 uTools <code>plugin.json</code>：<code>main</code>、<code>logo</code> 与文本型 <code>features</code> 会投影为 iHub 前端命令；<code>tools</code> 会按有界 JSON Schema 锁定，并由当前隐藏 runtime 的 <code>utools.registerTool</code> 注册，支持结构化调用、进度、取消、超时和生命周期清理。tools-only 包可只声明 <code>preload + tools</code>；声明的 preload 会纳入 SHA-256 快照，并从宿主内存路由作为无 Node 权限的普通 WebView 脚本加载。兼容层还提供 ready/enter/out 生命周期、宿主子输入框、动态功能、同步文档/附件数据库、取色与选区截图、显示器 API、经系统选择器授权的录屏源、受确认保护的本地 Shell 动作、Windows 原生文件图标，以及可配置 OpenAI-compatible Chat Completions 服务的 <code>utools.ai</code> / <code>utools.allAiModels</code>。AI Provider 的地址、模型和默认项由用户在偏好设置中管理，API key 只进入系统凭据保护的加密存储；流式正文、推理内容、函数工具调用、取消、轮次上限和插件生命周期清理均由 Rust 宿主控制。该沙箱不提供 <code>fs</code>、<code>child_process</code> 或其他 Node/Electron ambient authority；文件/图片 matcher 与浏览器可访问的本地搜索索引也不会因此开放。本地搜索继续完全由 Rust 原生索引实现。
